...
export async function fetchIssues(lang: Language): Promise<AppIssue[]> {
  ...
  // Try ordered query first; fall back to plain query if any column doesn't exist.
  let rawRows: DbIssue[] | null = null;
  ...
  if (!orderedResult.error && orderedResult.data) {
    rawRows = orderedResult.data as DbIssue[];
    console.log("[issues] loaded (ordered):", rawRows.length);
  } else {
    if (orderedResult.error) {
      console.error("[issues] load error (ordered):", orderedResult.error.message);
    }
    // Fallback: plain query with no server-side ordering
    ...
    if (fallbackResult.error || !fallbackResult.data) {
      console.error("[issues] load error (fallback):", fallbackResult.error?.message ?? "no data");
      return [];
    }
    rawRows = fallbackResult.data as DbIssue[];
    console.log("[issues] loaded (fallback):", rawRows.length);
  }

  // JS-side sort: newest first using any available date field, then id.
  const getMs = (r: DbIssue): number => {
    const v = r.published_at ?? r.publish_date ?? r.created_at;
    if (!v) return 0;
  ...
  console.log("[issues] first issue:", sorted[0]?.id ?? "none");
  return sorted;
}
