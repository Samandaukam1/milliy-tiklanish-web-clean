...
import { supabase, loadSupabaseProfile, upsertSupabaseProfileWithDetails, lookupEmailByLogin } from "@/lib/supabase";
...
export async function registerUser(input: RegisterUserInput): Promise<UserProfile> {
  ...
  const upsertResult = await upsertSupabaseProfileWithDetails(authUser.id, patch);
  let profile = upsertResult.profile;

  if (upsertResult.error) {
    console.error("[register] profile insert error:", upsertResult.error);
  }
